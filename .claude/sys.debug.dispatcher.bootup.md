...
|---|---|---|
| `~/lobster/` is on `local-dev` | `git -C ~/lobster branch --show-current` | Yes — send_reply to owner immediately |
...
When an invariant check fails in debug mode, **always alert the owner immediately**
