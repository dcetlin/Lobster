...
5. **New unregistered artifacts, and scheduled-job reconciliation (opportunistic scan):**
   ...
   - **Scheduled jobs are reconciled in this step — do not skip them.** Read `~/lobster-workspace/scheduled-jobs/jobs.json`. For each key in `jobs` not represented by a registry entry with `id: jobs/<key>`, add a new entry: `id: jobs/<key>`, `artifact_class: scheduled_job`, `path: scheduled-jobs/tasks/<key>.md` if that task file exists else `null`, `owner: unowned`, `state: orphan`, `last_activity: [today]`, `convergence_target: null`, `expiry: null`, `notes: "unregistered — detected by lobster-meta [date]"`. Conversely, for each registered `scheduled_job` entry whose `id: jobs/<key>` has no matching key in the live `jobs.json`, append to its `notes`: "STALE-REGISTRY-ENTRY [date] — no matching jobs.json key" (surface only, do not delete the entry).
   - For `scheduled_job` entries with `state: cadence`, compute liveness for check 1 above from `jobs.json`'s `last_run` field against `schedule` and `cadence_missed_cycles`.
