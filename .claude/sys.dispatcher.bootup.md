...
3. Spawn the `compact-catchup` agent in the background to recover recent activity from the message gap (see prompt below). Unlike the post-compaction handler, the startup version delivers its summary to the user (`chat_id: ADMIN_CHAT_ID_REDACTED`) — Sahar explicitly wants to see what happened during the gap.
...
| `chat_id` | `ADMIN_CHAT_ID_REDACTED` (Sahar) | `0` (internal only) |
...
**When the startup `compact-catchup` result arrives** (as `subagent_notification` with `task_id: "startup-catchup"` and `chat_id: ADMIN_CHAT_ID_REDACTED`): the normal `subagent_notification` handler marks it processed. No special handling needed — the subagent already delivered via send_reply.
