...
## Principle Weights in Subagent Dispatch

When composing a subagent prompt (any Task call that goes through the dispatch template gate), include an **Active Principle Weights** section after the task instructions. This section is generated by `format_principle_weights_block` from `src.orchestration.principle_weights`.

```python
from src.orchestration.principle_weights import assign_principle_weights, format_principle_weights_block

weights = assign_principle_weights(task_type)  # e.g. "debugging", "mobile_response", "design_inquiry"
weights_block = format_principle_weights_block(weights)
```

Append to the subagent prompt:

```
### Active Principle Weights (from task context)
{weights_block}
```

Where `weights_block` renders as (example for task_type="debugging"):
```
- pattern_perception: 0.7 — actively name structural patterns before conclusions
- structural_coherence: 0.6 — verify internal consistency before committing to output
- attunement_over_assumption: 0.9 — name causal vs. symptom layers explicitly
- minimal_cognitive_friction: 0.7 — lead with signal; key finding in paragraph 1
```

Principles with weight < 0.6 are omitted to avoid noise. The block is purely advisory — it guides the subagent emphasis, not its behavior contract.

Available task types: `debugging`, `mobile_response`, `design_inquiry`, `default`.
...
2b-pw. **Principle Re-activation** (session start):
    After loading bootup context, explicitly name which principles are live for this session:
    1. Read the five principles from `src.orchestration.principle_weights.PRINCIPLE_NAMES`
    2. State which two or three are highest-weight for the current session context
    3. For each named principle, state one concrete behavioral implication for this session
    Do not proceed to message processing until this step is complete.
