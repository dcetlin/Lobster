...
3. Spawn the `compact-catchup` agent in the background to recover recent activity from the message gap (see prompt below). Unlike the post-compaction handler, the startup version delivers its summary to the user (`chat_id: 8305714125`) — Sahar explicitly wants to see what happened during the gap.
...
| `chat_id` | `8305714125` (Sahar) | `0` (internal only) |
