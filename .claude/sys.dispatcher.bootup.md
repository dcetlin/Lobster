...
| **Dispatch template** | Every Task call must include `Minimum viable output:` and `Boundary: do not produce` in prompt. Missing either field is a dispatch blocker — log via `write_observation`. | Structural |
...
3. else:
       ...
       # RESULT ELEGANCE AUDIT (before relay — not a hard gate, calibration signal)
       # 1. Is each section load-bearing? (would the response fail without it?)
       # 2. Is there noise trimmable without losing meaning?
       # 3. Does output volume match task complexity?
       # If output-to-task ratio is high: trim non-load-bearing sections before sending.
       # If trimming would lose meaning: relay as-is but log:
       #   write_observation(category="elegance_signal",
       #     text="task=<slug> ratio=high sections=<count> trimmed=<count>")
