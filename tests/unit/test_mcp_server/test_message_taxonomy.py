...
class TestTaxonomyConstants:
    ...
    def test_wos_execute_registered_in_inbox_message_types(self):
        """wos_execute must be in INBOX_MESSAGE_TYPES so mark_processing does not warn.

        Before issue #856 fix, wos_execute was absent — causing mark_processing to log
        'WARNING: unknown message type wos_execute' and the dispatcher to fall through
        without calling route_wos_message(), leaving UoWs stuck in executing status.
        """
        from message_types import INBOX_MESSAGE_TYPES
        assert "wos_execute" in INBOX_MESSAGE_TYPES, (
            "wos_execute must be registered in INBOX_MESSAGE_TYPES; "
            "absence causes mark_processing to warn and the dispatcher to misroute"
        )

    def test_wos_execute_registered_in_inbox_system_types(self):
        """wos_execute is a system-generated type (produced by executor-heartbeat).

        It must appear in INBOX_SYSTEM_TYPES (not INBOX_USER_TYPES) because it is
        generated by the executor cron process, not sent by a human user.
        """
        from message_types import INBOX_SYSTEM_TYPES
        assert "wos_execute" in INBOX_SYSTEM_TYPES, (
            "wos_execute is system-generated and must be in INBOX_SYSTEM_TYPES"
        )

    def test_wos_execute_not_in_user_facing_types(self):
        """wos_execute must not be in USER_FACING_TYPES — it requires subagent spawn, not reply."""
        from message_types import USER_FACING_TYPES
        assert "wos_execute" not in USER_FACING_TYPES, (
            "wos_execute is a system dispatch message, not a user-facing message requiring a reply"
        )
