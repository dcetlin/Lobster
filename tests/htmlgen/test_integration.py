...
# ---------------------------------------------------------------------------
# wos_dashboard.py and wos_uow_detail_gen.py unmodified guard
# ---------------------------------------------------------------------------


class TestExistingGeneratorsUnmodified:
    """Confirm wos_dashboard.py and wos_uow_detail_gen.py still import and function."""

    def test_wos_dashboard_importable(self):
        """wos_dashboard must be importable without errors."""
        import src.orchestration.wos_dashboard as dash  # noqa: F401
        assert hasattr(dash, "upload_html") or hasattr(dash, "generate_html") or True
        # Key guard: the module imported without raising
    ...
    def test_wos_uow_detail_gen_importable(self):
        ...
        result = _sp.run(
            [
                ...
                "import src.orchestration.wos_uow_detail_gen as d; print('ok')"
            ],
            capture_output=True,
            text=True,
        ...
        assert result.returncode == 0, (
            f"wos_uow_detail_gen failed to import: {result.stderr}"
        )
        assert "ok" in result.stdout

    def test_wos_dashboard_has_upload_html_function(self):
        import src.orchestration.wos_dashboard as dash
        assert callable(getattr(dash, "upload_html", None)), \
            "wos_dashboard.upload_html must still exist"

    def test_wos_dashboard_uploads_dir_function_intact(self):
        import src.orchestration.wos_dashboard as dash
        assert callable(getattr(dash, "_uploads_dir", None)), \
            "wos_dashboard._uploads_dir must still exist"

    def test_wos_dashboard_bisque_base_url_function_intact(self):
        import src.orchestration.wos_dashboard as dash
        assert callable(getattr(dash, "_bisque_base_url", None)), \
            "wos_dashboard._bisque_base_url must still exist"

    def test_renderer_does_not_modify_wos_dashboard_module(self):
        """Importing renderer must not change wos_dashboard's interface."""
        import src.orchestration.wos_dashboard as dash_before
        ...
        assert dash_after.upload_html is upload_before, \
            "renderer.py must not modify wos_dashboard.upload_html"
