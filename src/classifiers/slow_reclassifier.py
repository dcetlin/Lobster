...
def read_recent_events(conn: sqlite3.Connection, hours: int = LOOK_BACK_HOURS) -> list[EventRow]:
    ...
    but do NOT yet have a slow-v1 tag, and are not synthetic pattern_observation events.

    Excludes:
    - Events already tagged by slow-v1 (idempotency: avoid re-processing every cycle).
    - Events of type 'pattern_observation' (written by this classifier as outputs —
      re-classifying them creates a self-reinforcing loop where pattern_observation
      events get tagged meta_reflection and trigger further pattern detections).
