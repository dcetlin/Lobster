...
**Quality finding:**
- **`_call_write_observation` uses the wrong protocol.** The hook POSTs `{"jsonrpc": "2.0", "method": "tools/call", ...}` to `http://localhost:8765/mcp`. The MCP server at that endpoint uses `StreamableHTTPSessionManager(stateless=False)` — a stateful session protocol requiring MCP initialization handshake before tool calls are accepted. A bare JSON-RPC POST will fail with a protocol error. The exception is silently caught and written to hook-failures.log. The correct approach is `lobster-observe.py` or direct inbox file write (the established pattern used by cron-direct scripts) — `scripts/lobster-observe.py` already implements `write_observation_to_inbox()` which writes directly to `~/messages/inbox/` producing a `subagent_observation` message the dispatcher processes normally. The tests mock `_call_write_observation` entirely, so they pass without exposing this defect.
- **`_find_message_file` search order is correct.** At PostToolUse time, `mark_processed` has already completed — the file is in processed/. The fallback to processing/ and inbox/ covers the narrow race where PostToolUse fires before the rename completes. The send_reply atomic path (send_reply with message_id bypasses `mark_processed` entirely) does not produce a PostToolUse hook on `mark_processed`, so there is no false-coverage gap here.
- **Migration 84 jq pattern is correct.** The idempotency check (`[.hooks.PostToolUse[]?.hooks[]?.command // empty] | map(select(contains("wos-execute-gate"))) | length`) uses the same structure as Migration 75. The `.hooks.PostToolUse = (.hooks.PostToolUse // []) + [...]` append pattern does not clobber existing array entries. The double-condition `= "0" || = ""` correctly handles both numeric-zero and empty-string jq output. `python3` in the command string is technically against the `uv` convention (CLAUDE.md) but consistent with all existing hooks and migrations in this codebase — advisory only.
- **`check_gate` is a clean pure function.** The isolation of I/O (file lookup, log write, HTTP call) from the gate logic is correct. The test suite exercises the pure function thoroughly and covers gate pass, gate miss, edge cases (empty type, empty `_processing_started_at`, exact-match-only for `wos_execute`). Test design is consistent with the test learnings.md entries regarding isolation.

**Patterns introduced:** PostToolUse-as-observation-layer for compaction-vulnerable gates. The hook demonstrates the pattern of coupling `_processing_started_at` field presence (set by `mark_processing`) as a structural signal detectable at PostToolUse time. This pattern is reusable for other two-step protocol gates.

**What this forecloses:** Nothing architecturally foreclosed. The observation-only approach leaves open a future PreToolUse upgrade that blocks on gate miss. The broken `_call_write_observation` HTTP path can be fixed by replacing the urllib call with a direct inbox write — this fix is additive, not a redesign.

**Opportunity cost note:** The missing structural prevention (PreToolUse blocking) is the issue #855 description's stated intent. Whether observation-only is acceptable for the dispatcher starvation failure mode is a product decision. The current PR reduces gap-to-detection, not gap-to-prevention.

**VERDICT: NEEDS_CHANGES**

**Revision contract:**
- **Gap 1: write_observation broken protocol** — Replace `_call_write_observation`'s urllib JSON-RPC POST with a direct inbox file write using the pattern from `scripts/lobster-observe.py` (`build_observation_payload` + `write_observation_to_inbox`), or call `lobster-observe.py` as a subprocess. Addressed when: the hook's gate-miss side effect can actually surface to the dispatcher without requiring MCP session establishment.

---

### [2026-04-22] PR #854 — fix: register wos_execute in message type routing (starvation root cause fix)

**Vision alignment:** Stage 1 adversarial prior: the PR may be solving the symptom (the `mark_processing` WARNING) rather than the structural root cause (dispatcher not calling `route_wos_message` after context compaction). The diagnosis itself characterizes the type-registry addition as "the single lowest-risk, highest-leverage fix to investigate first" — not a confirmed root cause fix. What would have to be true for this to be wasted effort: the dispatcher's routing decision for `wos_execute` is not gated on `INBOX_SYSTEM_TYPES` membership but on behavioral prose that compaction erases, in which case registering the type silences a warning without changing routing. Stage 2 confirms this concern is real but does not overturn the fix: `inbox_server.py` already formats `wos_execute` messages with a `dispatcher_hint` (line 5197-5199) independent of type registration, and the `mark_processing` warning is explicitly non-blocking (line 5594: "Non-blocking: unknown types are logged as a warning but not rejected"). The registration is therefore not the structural gate the PR description implies — but it is not inert either. The `dispatcher_hint` injection at check_inbox/wait_for_messages time requires the message to be recognized in the format branch (line 5175), which it already was. The causal chain "missing type registration → dispatcher falls through to no-op" is not mechanically traceable through the code: the formatter already branched on `wos_execute` hardcoded strings before this PR. The true root cause of starvation (Failure Mode A: `mark_processed` called directly without `mark_processing`, suggesting the dispatcher treated messages as `subagent_notification`) is not addressed by type registration — it is addressed by the `WOS Execute Gate` in CLAUDE.md remaining in dispatcher behavioral context, which is compaction-vulnerable. The structural fix the diagnosis recommends (PostToolUse hook or pre-merge-gate equivalent) is still outstanding. Vision operating principle-3 ("Determinism over judgment for conditionals — if-then logic and field checks are code, not LLM instructions") is the relevant anchor: the type registration is data, but the routing decision still lives in dispatcher prose.

**Alignment verdict:** Confirmed

**Quality finding:**
- **The type registration is correct placement and correct taxonomy.** `wos_execute` is system-generated (executor-heartbeat produces it, not a user), so `INBOX_SYSTEM_TYPES` is the right set. `USER_FACING_TYPES = INBOX_USER_TYPES` means registering in SYSTEM_TYPES correctly excludes it from user-facing paths (observation queuing at line 5612, user model context at line 5622) — this is a real, correct behavior difference. The registration eliminates noisy WARNINGs that could confuse future debugging.
- **The causal chain in the PR description overstates the structural effect.** The PR claims registering in `INBOX_SYSTEM_TYPES` "allows the dispatcher's type-based gate to fire structurally via WOS_MESSAGE_TYPE_DISPATCH." This is incorrect: `WOS_MESSAGE_TYPE_DISPATCH` in `dispatcher_handlers.py` was already complete and already mapped `wos_execute → handle_wos_execute`. The dispatcher's behavioral gate (CLAUDE.md `WOS Execute Gate`) does not read `INBOX_SYSTEM_TYPES` at dispatch time — it reads the message type from the formatted `check_inbox` output. The type was already formatted correctly (line 5175-5177 and 5197-5199 in `inbox_server.py`). The registration fix is necessary but not sufficient for structural routing.
- **The routing tests correctly exercise the dispatch contract.** `TestRouteWosMessage` tests verify: `wos_execute` in `WOS_MESSAGE_TYPE_DISPATCH` (registration precondition), `action == "spawn_subagent"` (dispatcher obligation), task_id format, prompt content (uow_id and instructions embedded), agent_type default/override, ValueError on unknown type, and purity. These tests are well-scoped. One gap: no test verifies the behavior when required fields (`uow_id`, `instructions`) are absent from the message — the KeyError that would result is documented in the docstring but not exercised. This is an advisory observation, not a defect.
- **The UoW reset to `proposed` is the right call.** 8 UoWs stuck in `executing` past `timeout_at` with no heartbeat will not self-recover for ~24h (TTL uses `started_at`, not `timeout_at`). Manual reset with audit log entries is the correct operational response. The diagnosis verified `0 UoWs remaining in executing` post-reset. The separate TTL gap (`timeout_at IS NOT NULL AND timeout_at < NOW()`) named in the diagnosis remains open as issue #849 dependency.

**Patterns introduced:** Type registration with inline comment documenting the calling obligation (`dispatcher must call route_wos_message()`) is a useful pattern for system message types with non-obvious handler requirements. The comment at the registration site creates a single-file breadcrumb that survives code search even when CLAUDE.md has compacted.

**What this forecloses:** Nothing architecturally foreclosed. The structural fix (PostToolUse hook enforcing that a Task call follows `mark_processing` for `wos_execute` messages) remains unbuilt and is the correct long-term resolution per operating principle-3. This PR does not foreclose that work.

**Opportunity cost note:** The fix is narrowly scoped (1 line of production code, 143 additions including tests) and directly on the `current_focus` primary item ("fix executor dispatch starvation"). The structural hook fix is the correct next step — this PR is the necessary prerequisite that silences noise and confirms the taxonomy placement.

**VERDICT: APPROVED**

---

### [2026-04-22] PR #852 — fix: classify_uow phrase-level seed matching (false positive fix)

**Vision alignment:** The adversarial prior — this implementation is solving the wrong problem, or solving the right problem in a direction that forecloses better paths — has a specific surface here: if `close_reason` strings are unstructured agent-authored free text with no enforced convention, phrase-level regex will remain brittle against novel phrasings not covered by the current patterns. The deeper fix would be a structured field at write time rather than a more complex reader. However, this scenario does not disqualify the PR — it names a direction for future improvement (structured close_reason at agent write time), not a reason to reject the current fix. The `what_not_to_touch` carve-out in vision.yaml explicitly excludes "new detection or classification rules" while permitting WOS-related classification work. Operating principle-5 ("discriminator improvement over rule addition") directly supports this change: this is tightening an existing discriminator's specificity, not adding a new rule category. The direction is correct and in-scope.

**Alignment verdict:** Confirmed

**Quality finding:**
- **Phrase-level patterns are correctly scoped and directionally better.** The issue creation pattern `(opened|created|filed|new|spawned)\s+issue\s*#?\d*` is a material improvement over bare `"issue"` substring matching. `\d*` (zero-or-more) for the issue number is intentionally permissive and correct — agents may not always include a number. The UoW spawn pattern retaining the "spawn" infinitive is also correct for agent-authored close_reason prose.
- **The `new` keyword warrants a low-confidence note.** `new\s+issue` as a fragment can still match incidentally in free-text close_reason values (e.g., "found a new issue with the approach"). The risk is lower than bare "issue" but not zero for unstructured prose. This is an acceptable residual risk for a daily digest cron job but should be monitored if digest counts remain suspicious.
- **Test for `opened issue #789 for follow-up` is ambiguous.** The close_reason also contains "follow-up", which independently triggers OUTCOME_SEED via the bare keyword path. The test passes for the right overall result but does not isolate whether the regex fired. The cleaner proof is `test_creation_phrase_created_issue_is_seed` ("created issue #100 to track regressions"), which has no bare seed keyword trigger — that test definitively exercises the regex. Both tests together are sufficient coverage, but the first is not a clean isolated proof of the regex path. This is a test hygiene observation, not a defect.
- **Stale test correction is handled correctly.** The two inverted tests (previously asserting SEED based on buggy behavior) are updated to HEAT and PEARL respectively — consistent with the learnings.md PR #827 pattern (stale assertions must be corrected, not carried through merge). The PR does not delete these tests but corrects them into regression guards for the false-positive cases, which is the right resolution.

**Patterns introduced:** Phrase-level regex with creation-verb anchoring as the discriminator for semantic classification of unstructured prose fields. Bare keyword matching retained only for strings specific enough to be self-describing (seeded, follow-up, follow_up). This creates a two-tier detection approach: anchored patterns for ambiguous keywords, bare matching for unambiguous ones — a clean and extensible structure for future seed keyword additions.

**What this forecloses:** Nothing architecturally foreclosed. The underlying brittleness of unstructured close_reason matching is not resolved by this PR — it is reduced. Structured close_reason fields at agent write time remain the long-term direction if classification precision becomes a reliability concern.

**Opportunity cost note:** Narrowly scoped fix to a daily cron job classifier. Negligible opportunity cost relative to current_focus items.

**VERDICT: APPROVED**

---

### [2026-04-22] PR #848 — feat: WOS heartbeat-based UoW locking (replaces fixed 4h TTL)

**Vision alignment:** The adversarial prior entering Stage 1 — this implementation is solving the wrong problem, or solving the right problem in a direction that forecloses better paths — has a surface to engage here, and it is the sequencing gap. This week's current_focus is "WOS execution health: fix executor dispatch starvation, resolve RALPH Cycle 7 failures." The heartbeat design solves a distinct failure mode (agent death invisible for 4h) rather than executor starvation. The question is whether this is the right path right now, or whether it addresses a future problem while the present problem (starvation, RALPH Cycle 7 failures) remains unresolved. What would have to be true for this to be the wrong path: the executor starvation root cause is not related to TTL behavior at all, in which case this PR ships an improvement to a non-critical path while blocking attention on the critical one. What is confirmed: the design doc is coherent, the problem it solves is real at scale, and principle-1 (structural prevention) and principle-2 (keep the correction path open) both favor heartbeat over TTL. The sequencing concern is partially neutralized by a key architectural fact: no subagent currently calls `write_heartbeat()`. This means Phase 2b ships harmlessly — it finds nothing until agent-side integration is done. The detection layer is additive and produces no behavior change for existing UoWs (backward-compatible via NULL heartbeat_at). The 24h TTL safety net is also retained. The premise that this forecloses attention on starvation holds only if the engineering bandwidth cost is non-trivial; the PR's scope (1014 additions, 21 tests, clean module boundary) suggests it was executed without diverting from the critical path. Alignment is confirmed with a noted sequencing observation: the value of this PR is latent until `write_heartbeat()` is integrated into subagent prompt templates.

**Alignment verdict:** Confirmed

**Quality finding:**
- **Claim transaction atomicity is correct.** The Step 5b heartbeat initialization (both `heartbeat_at` and `heartbeat_ttl`) occurs inside the 6-step claim transaction, before `conn.commit()`. The audit log INSERT in Step 6 includes `heartbeat_ttl` in the JSON payload. However, there is dead code immediately preceding the executed assignment: `heartbeat_ttl = max(300, min(timeout_seconds // 4, 300))` is computed and then unconditionally overwritten by `heartbeat_ttl = 300`. The first line is never used. This is not a correctness defect (the final value is 300 either way), but the commented intent to support per-UoW tuning via `estimated_runtime` is structurally undermined — the code signals future intent that is immediately abandoned. The comment and dead assignment should be removed to avoid misleading future contributors.
- **SQL correctness for stale detection is sound, with one note.** The `julianday()` arithmetic in `get_stale_heartbeat_uows` is SQLite-idiomatic and correct for the purpose. The boundary tests verify both sides of the threshold (stale at ttl+buffer+2s, fresh at ttl+buffer-2s). The `CAST(...AS INTEGER)` truncates the computed silence duration to integer seconds — this means a UoW exactly at `heartbeat_ttl + buffer_seconds` (fractional seconds edge) could be classified as fresh if the fractional part pushes the floor value below the threshold. This is an acceptable rounding behavior and consistent with the +30s buffer purpose. The `from_status` hardcoding to `'active'` in the audit entry when the UPDATE covers `status IN ('active', 'executing')` is a pre-existing inaccuracy confirmed by the PR description and not introduced here.
- **Inline test copy risk is real and acknowledged.** The `_recover_stale_heartbeat_uows` function in the test file is a faithful copy of the production function, required because `steward-heartbeat.py` cannot be imported in CI due to a transitive `src.ooda` dependency. The test comment correctly states "any behavioral change to the production function must be reflected here." This is a synchronization debt, not a correctness defect at merge time. The `adversarial prior as detection precondition` golden pattern (oracle vocabulary) increases the weight given to this observation: the inline copy introduces a class of silent drift where tests can diverge from production. The pattern named in learnings.md as "interface migration without updating all callers" (PR #720) applies as a risk surface here — future changes to the production function will require a deliberate two-file update that the tooling does not enforce. This is the most significant ongoing risk the PR introduces.
- **Test coverage is thorough for the registry layer.** Migration column presence, write_heartbeat update and rejection, get_stale detection at both boundary sides, NULL exclusion, status filtering, record_heartbeat_stall transition and audit content, race safety (returns 0 on already-advanced UoW), recover function dry_run semantics, and count accuracy are all verified. The test for claim initialization tests `set_heartbeat_ttl` (the registry method) rather than the full executor claim chain — this is a valid adaptation given the import constraint, and the PR is honest about the gap (comment in test class explains the workaround). One gap: the dead code path in executor.py (`heartbeat_ttl = max(300, min(...))` followed by unconditional override) is not tested because it cannot be reached — the test-by-SQL-inspection approach verifies the outcome (heartbeat_ttl=300 written) but cannot detect the unreachable computation.

**Patterns introduced:** The two-path observation loop architecture (heartbeat path for UoWs with non-NULL heartbeat_at, TTL path for legacy UoWs) is a clean backward-compatible split. New UoWs get signal-driven detection; pre-migration UoWs continue on time-based TTL. This is the `seam-first abstraction` golden pattern applied to the observation loop: the NULL/non-NULL heartbeat_at field is the explicit seam between the two paths. The pattern is correct and should be maintained as the heartbeat adoption grows.

**What this forecloses:** The `heartbeat_ttl = max(300, min(timeout_seconds // 4, 300))` dead code signals an intent to support per-UoW TTL tuning that is immediately deferred. Shipping with this dead code makes future tuning a follow-on that touches executor.py — the seam is correct but the intent-signal is noisy. Nothing architecturally foreclosed.

**Opportunity cost note:** The primary current_focus item is executor dispatch starvation, not TTL behavior. This PR is orthogonal to that constraint and ships a latent capability (no behavior change until agents integrate write_heartbeat). The sequencing is defensible but the activation dependency (agent-side heartbeat integration) should be tracked as a required follow-on before the feature delivers value.
...
- **Gap 1 (Broken env var test — `_DEFAULT_CLASSIFIER_PATH` computed at import time):** ADDRESSED. The module-level constant `_DEFAULT_CLASSIFIER_PATH` has been removed. In its place, `_default_classifier_path()` is a function that calls `os.environ.get("WOS_CLASSIFIER_YAML")` at invocation time. The `classify_posture` default-path resolution changed from `classifier_path or _DEFAULT_CLASSIFIER_PATH` to `classifier_path if classifier_path is not None else _default_classifier_path()`. The `if classifier_path is not None` form is correct — the prior `or` idiom would also have bypassed the helper for a `classifier_path=""` empty string, which is not a concern in practice but the explicit None check is the tighter specification. `test_env_var_overrides_default_classifier_path` now calls `classify_posture({"type": "seed"})` with no `classifier_path` argument, so `_default_classifier_path()` is called at test time, `monkeypatch.setenv` has taken effect by then, and the env var resolves to the fixture YAML. The test now exercises the code path it claims to test.

- **Gap 2 (Assertion softening in test_registry.py):** ADDRESSED. The loosened `assert row["route_reason"] != "phase1-default: no classifier"` assertion has been replaced with a controlled fixture approach: the test writes `_TEST_CLASSIFIER_YAML` (a catch-all YAML with `route_reason_template: "classifier-test: catch-all matched"`) to `tmp_path`, sets `WOS_CLASSIFIER_YAML` via `monkeypatch.setenv`, and asserts `row["route_reason"] == "classifier-test: catch-all matched"`. Silent fallback (`"classifier-unavailable: defaulting to solo"`) produces a different string and now fails the assertion. The revision contract criteria (a) is satisfied: the revision shows the specific expected value asserted.

**Vision alignment:** This is a re-review of a two-commit test remediation. The adversarial prior — this is solving the wrong problem, or solving the right problem in a direction that forecloses better paths — has no surface to engage here. Both changes are confined to the test layer and one production function's internal default-path resolution. No new direction is introduced. The three underlying fixes (UoWStatus.CANCELLED, classifier wiring, upgrade.sh syntax) were confirmed in the original entry as correctly scoped. The lazy-lookup change is a minor behavioral improvement for long-running processes: env var overrides now take effect at call time without module reload, which is consistent with the documented intent of `WOS_CLASSIFIER_YAML`. The original vision alignment finding stands unchanged.

**Alignment verdict:** Confirmed

**Quality finding:**
- **Gap 1 fix is correct and complete.** The lazy `_default_classifier_path()` helper reads `os.environ.get("WOS_CLASSIFIER_YAML")` at call time, making the env var effective for both test monkeypatching and runtime env changes without restart. The `_CLASSIFIER_YAML_DEFAULT_FALLBACK` constant is correctly isolated as an import-time Path construction (home directory resolution is stable). No other callers of `_DEFAULT_CLASSIFIER_PATH` exist in the codebase, so the rename introduces no regression surface.
- **Gap 2 fix is correct and complete.** The `_TEST_CLASSIFIER_YAML` fixture is minimal (one catch-all rule, conditions empty list, deterministic `route_reason_template`). The `monkeypatch.setenv` + lazy-lookup path means the env var set in `test_insert_new_proposed_record` reaches `_default_classifier_path()` inside `classify_posture`, which is called from `_upsert_typed` inside `registry.upsert()`. The wiring chain is exercised end-to-end: registry call → upsert → germination → classifier call → fixture YAML loaded → catch-all matched → specific string written to DB → asserted. Silent fallback produces `"classifier-unavailable: defaulting to solo"`, which fails the exact-match assertion. This satisfies the original gap's requirement that CI can now distinguish correct wiring from silent fallback.
- **No new issues introduced.** The diff touches exactly two files (`routing_classifier.py` and `test_registry.py`), both within scope of the revision contract. No feature bundling. The `textwrap.dedent` import in `test_registry.py` is a correct dependency for the multiline YAML fixture string. The `tmp_path` and `monkeypatch` fixture parameters added to `test_insert_new_proposed_record` are standard pytest patterns with no side effects on other tests.
- **The `confidence` field absence (noted in the original entry as a partial application of the PR #602 golden pattern) is unchanged.** This was not a gap requiring revision — it was a "patterns introduced" observation. It remains an improvement opportunity for a future PR.

**Patterns introduced:** The controlled test fixture pattern for classifiers — write a minimal YAML with deterministic rule output, point `WOS_CLASSIFIER_YAML` at it, assert the specific string — is a reusable testing approach for any component that reads the classifier at runtime. It is the correct application of the learnings.md PR #839 entry ("assertion softening to not-legacy-value cannot distinguish wired-correctly from silently-failed-with-fallback").

**What this forecloses:** Nothing. The test fixture approach is additive; the lazy-lookup change does not constrain future classifier implementations.

**Opportunity cost note:** Both fixes were minimal and targeted. No alternative paths were foreclosed.

**VERDICT: APPROVED**

---

### [2026-04-22] PR #839 — fix: WOS small issues — UoWStatus enum, classifier wiring, upgrade.sh syntax

**Vision alignment:** The adversarial prior — this is solving the wrong problem, or solving the right problem in a direction that forecloses better paths — does not land on any of the three changes in isolation, but the classifier wiring change bears scrutiny. The current_focus constraint in vision.yaml is "WOS execution health: fix executor dispatch starvation, resolve RALPH Cycle 7 failures, drain 106 proposed UoWs." The theory is that these three defects (missing enum value, unwired classifier, broken upgrade.sh syntax) are live blockers to that goal. The `UoWStatus.CANCELLED` gap is clearly a crash risk on deserialization of any cancelled UoW. The `upgrade.sh` syntax break is clearly blocking all upgrades (`bash -n` non-zero means the file cannot be sourced). The classifier wiring is the question: vision.yaml `current_focus.horizon` explicitly names "connect vision_ref routing rules so route_reason cites vision.yaml fields" as what comes next after pipeline flows. Wiring the classifier at germination is a prerequisite to that goal — it is "integration rate before new feature rate" (principle-4). What would have to be true for this to be the wrong path: if the starvation failure is not caused by posture misclassification but by dispatcher/executor dispatch gaps, classifier wiring is neutral to the immediate crisis. The PR description is honest about this: "Executor dispatch path is unchanged — posture classification happens at germination, not dispatch." The classifier wiring is not fixing starvation; it is fixing a pre-existing defect that would otherwise persist. That framing is compatible with the vision.

**Alignment verdict:** Confirmed

**Quality finding:**
- **UoWStatus.CANCELLED** is a clean, narrowly-scoped fix. The enum was missing a value that lived in production DB rows and SQL queries, causing `ValueError` on deserialization. Adding it and including it in `is_terminal()` is correct. The comment update removing "legacy status not in enum" is accurate.
- **routing_classifier.py** is wired correctly: the `from orchestration.routing_classifier import classify_posture` inside `_upsert_typed` means the classifier fires at germination. The broad `except Exception` fallback in `_upsert_typed` is intentional defense-in-depth and is not deceptive (the comment does not describe re-raise). The lazy `import yaml` inside `_load_classifier_yaml` is a valid optional-dependency pattern. However, the env var test `test_env_var_overrides_default_classifier_path` is broken: `_DEFAULT_CLASSIFIER_PATH` is a module-level constant computed at import time from `os.environ.get(...)` — `monkeypatch.setenv` does not recompute it. The test passes in CI because `classify_posture` receives `classifier_path=None`, which resolves to the already-computed constant, not the monkeypatched env var. The env var override feature either does not work as documented or is untested.
- **test_registry.py assertion softening** applies the pattern named in learnings.md PR #738: the assertion changed from exact value to "not the legacy placeholder." This now passes whether the classifier fires successfully or falls back with `"classifier-unavailable: defaulting to solo"` — so CI cannot distinguish correct wiring from silent import failure. A test that controls `WOS_CLASSIFIER_YAML` and asserts the specific catch-all rule reason would close this.
- **upgrade.sh** syntax fix is mechanically correct (`bash -n` now exits 0) and the `migrated=$((migrated + 1))` counter increment completes the migration block structure.

**Patterns introduced:** The `ClassifierResult` frozen dataclass partially applies the golden pattern from PR #602 (classification result as typed frozen dataclass with observability fields). `rule_name` provides gate_matched equivalent. `confidence` is absent — the PR #602 pattern explicitly called this field out for enabling human-review flagging when classification is uncertain. The catch-all rule and the fallback route are both returned as `rule_name="default"` vs `rule_name=None`, which provides some distinguishability, but no machine-readable confidence signal for the Steward to surface.

**What this forecloses:** Nothing structural. The classifier is wired via lazy import inside a try/except, so it can be replaced or extended without modifying the calling contract. The fallback chain (`YAML absent → fallback`, `no rule matches → fallback`) means any future YAML structure change degrades gracefully.

**Opportunity cost note:** The three changes are independent defects. Bundled correctly (PR description names all three explicitly) — this is not feature-bundling as named in the learnings. The opportunity cost question is whether classifier wiring should wait until the executor starvation is resolved (per vision.yaml horizon ordering). The PR description correctly characterizes this as a pre-existing defect fix, not a new feature, so the ordering concern is resolved by framing.

**VERDICT: NEEDS_CHANGES**
**Revision contract:**
- **Gap 1: Broken env var test** — `test_env_var_overrides_default_classifier_path` uses `monkeypatch.setenv` to override `WOS_CLASSIFIER_YAML` but `_DEFAULT_CLASSIFIER_PATH` is already computed at module import time. The test gives false confidence in the env var feature. Resolution options: (a) redesign `_DEFAULT_CLASSIFIER_PATH` to be computed lazily (callable, not constant), or (b) import the module fresh inside the test using `importlib.reload`, or (c) delete the test and document that env var override is a deployment-time feature tested manually. Option (c) is addressed if accompanied by a docstring note. Must not merge with the test passing when the feature it claims to test does not work.
- **Gap 2: Assertion softening in test_registry.py** — the loosened assertion passes whether the classifier fires or silently falls back. Replace with an assertion that controls the classifier path via `WOS_CLASSIFIER_YAML` monkeypatch (or skips if env var overhead is too high) and asserts a specific expected value from the classifier, or assert `!= "phase1-default: no classifier" AND != "classifier-unavailable: defaulting to solo"` so the test fails if the classifier import fails silently. The current assertion does not distinguish correct wiring from silent failure.
...
### [2026-04-22] PR #845 — fix: germinator philosophical false positive + executor UoW close-out protocol

**Vision alignment:** The adversarial prior — this implementation is solving the wrong problem, or solving the right problem in a direction that forecloses better paths — finds different traction on each of the two changes in this PR.

For the germinator fix: the "clearing" false positive caused live routing failures (engineering issues dispatched to frontier-writer instead of functional-engineer). This is a discriminator defect, not a classification philosophy problem. Principle-5 in vision.yaml ("Discriminator improvement over rule addition") is the exact structural anchor for this change. The fix improves the discriminator (removes an over-eager single-word term, adds two high-specificity replacements) rather than adding compensating rules downstream. No foreclosure risk — the philosophical register remains intact, the routing logic is unchanged.

For the close-out protocol: the adversarial prior lands here more firmly. The current_focus in vision.yaml is "WOS execution health: fix executor dispatch starvation, resolve RALPH Cycle 7 failures, drain 106 proposed UoWs." The close-out comment is observability infrastructure, not execution health. More specifically: the PR explicitly states that `result_text` will be None at all current call sites (inbox_server.py passes only two positional args), so every close-out comment will classify as "seed" with no summary text. The feature is structurally complete but semantically empty in production today. The Stage 1 question is whether this is the right time to build this. What would have to be true for this to be the wrong path: if the close-out comment adds subprocess overhead to the UoW completion path with no current signal value, and if engineering attention should concentrate on starvation. The PR is honest about this limitation. What prevents Stage 1 from hardening into a "Misaligned" finding: the feature is backward-compatible, degrades gracefully, does not touch the critical execution path (inbox_server.py unchanged), and closes a genuine observability gap — an engineer looking at a source GitHub issue had no way to know the UoW ran. That is a real problem even when result_text is None.

Pattern citations with behavioral effect: learnings.md PR #826 ("external API once per proposed UoW per cycle grows linearly") caused direct examination of the gh subprocess volume. Unlike PR #826, this fires once per UoW *completion* event, not per cycle per proposed UoW — volume is bounded by throughput, not backlog. This changed the assessment from "possible rate-limit concern" to "acceptable." Learnings.md PR #712 ("feature-bundling") caused explicit verification that both changes are named in the PR description — they are. Without the learnings pattern, the bundling check would not have been applied, and the germinator change would have been treated as the only subject of review.

**Alignment verdict:** Confirmed

**Quality finding:**
- **Germinator fix is correct and complete.** Removing "clearing" and replacing with "lichtung" (German, no false positive surface in English engineering text) and "ontological clearing" (phrase-level compound, correct substring-match treatment) closes the false-positive gap with high specificity. The partition of `_PHILOSOPHICAL_TERMS` into `single_word_terms` and `multi_word_terms` frozensets is computed inside `_is_philosophical` on every call — this is a minor efficiency concern (the term set is small and fixed, but the partition is recomputed on every invocation), but not a correctness issue. The regression test class `TestClearingFalsePositiveRegression` covers the live failure case plus two additional engineering-context variations. The positive tests for "lichtung" and "ontological clearing" verify the replacements fire correctly. Boundary concern: "ontological clearing" is matched via `phrase in combined` substring match. A sentence like "the ontological clearing of technical debt" would match; this is a real edge case where a metaphorical use of a philosophical phrase in an engineering context could produce a false positive. The risk is low but non-zero. Detection: the agent flagged this concern in the PR submission; the verdict is that the risk is acceptable given the low frequency of the phrase in engineering text.
- **UoW close-out protocol is structurally sound.** The pure/side-effecting decomposition (`classify_uow_output`, `_extract_github_issue`, `_build_closeout_comment` as pure functions; `_post_github_comment` isolated as the sole side-effecting function) correctly applies the functional-boundary pattern. The `gh_bin` injection parameter enables full test isolation without mocking subprocess at the module level. The non-fatal exception handling in `_post_github_comment` is correct: `subprocess.CalledProcessError` is caught specifically first, then the broad `Exception` catch handles unexpected failures (timeout, permission error, binary absent). The `subprocess.TimeoutExpired` is caught under the broad `Exception` clause — acceptable given that the function's contract is "non-fatal on any failure." Sequencing is correct: registry transition happens before the close-out comment, so a comment failure never blocks state machine progress.
- **The `result_text=None` default at all current call sites is an intentional design choice, not a defect.** The PR states this explicitly and the test for `result_text=None` (`test_empty_result_defaults_to_seed`) confirms the behavior is tested. The operational consequence is that every production close-out comment today will read `**Output type:** seed` and `**Summary:** ` (empty). This is a known limitation — it makes the close-out comment low-signal until inbox_server.py is updated to pass `result_text`. That wiring is a clear follow-on task.
- **`LOBSTER_WOS_REPO` default hardcodes "dcetlin/Lobster"** in `_extract_github_issue`. This is the correct production repo for this deployment. If a future installation uses a different repo slug, the default will silently post comments to the wrong repo. The env var override path is tested (`test_uses_lobster_wos_repo_env_var`). The risk is acceptable for a single-instance deployment; it should be noted as a configuration requirement in any multi-instance scenario.

**Patterns introduced:** The pure/side-effecting decomposition in wos_completion.py — pure builders (`classify_uow_output`, `_build_closeout_comment`, `_extract_github_issue`) feeding a single side-effecting orchestrator (`_post_closeout_comment_if_github`) — is a clean functional-boundary pattern that should propagate to future close-out or notification functions in this module. The `gh_bin` injection pattern for subprocess testability is already established in the codebase; this PR applies it consistently.

**What this forecloses:** Nothing structural. The close-out comment is additive; it can be extended when `result_text` is wired from inbox_server.py. The germinator's partition logic makes future multi-word philosophical term additions straightforward. The hardcoded `"functional-engineer"` default for `agent_type` (with the `hasattr(uow, "register")` override) is a minor coupling to the UoW struct's field name, but it is not a foreclosure — the UoW struct's `register` field is stable.

**Opportunity cost note:** The close-out protocol adds ~190 lines to wos_completion.py that are operationally inactive until inbox_server.py is updated to pass `result_text`. The germinator fix is the directly impactful change in this PR. The close-out protocol is preparatory work. Both are legitimate given the priority framing, but the close-out feature's full value is deferred to the inbox_server.py wiring.
...
### [2026-04-22] PR #846 — fix: executor routes agent type by UoW register (closes #842)

**Vision alignment:** The adversarial prior — this implementation is solving the wrong problem, or solving the right problem in a direction that forecloses better paths — finds no foothold here. The defect is concrete: `_select_executor_type` used keyword-matching on the UoW summary string, making the `register` field (classified at germination time by the Germinator) irrelevant to dispatch. A philosophical UoW with "fix" in its summary routed to `functional-engineer`. The fix replaces the keyword heuristic with a declarative dict lookup on the register field, which is exactly what vision.yaml principle-3 requires: "if-then logic and field checks are code, not LLM instructions." The `_REGISTER_TO_EXECUTOR_TYPE` table is an Encoded Orientation decision under constraint-3: it is traceable to `active_project.phase_intent` ("the Registry knows what work exists... giving routing decisions a traceable structural anchor") and is consistent with the mismatch gate design (the gate checks `_REGISTER_COMPATIBLE_EXECUTORS`, which this PR correctly extends). Routing by register does not foreclose any structural path — it closes the gap between germination-time classification and dispatch-time routing that the WOS substrate requires.

**Alignment verdict:** Confirmed

**Quality finding:**
- **Routing table correctness is sound.** `_REGISTER_TO_EXECUTOR_TYPE` maps operational and iterative-convergent → functional-engineer, human-judgment → lobster-generalist, philosophical → lobster-meta, unknown → lobster-generalist (with warning). The `_REGISTER_COMPATIBLE_EXECUTORS` is updated in sync (philosophical now accepts `lobster-meta`; human-judgment now accepts `lobster-generalist`), so the mismatch gate does not false-fire on normal routing. Backward compatibility is maintained: `msg.get("agent_type", "functional-engineer")` in `route_wos_message` and `agent_type="functional-engineer"` default in `_dispatch_via_inbox` handle messages written before this PR.
- **`_ASYNC_EXECUTOR_TYPES` expansion is correct.** `lobster-generalist` and `lobster-meta` both use the inbox dispatch path (via `_make_inbox_dispatcher`). Adding them to the async set means the executor transitions to `executing` at dispatch time and waits for `write_result` to fire `complete_uow` — the same semantics as `functional-engineer`. This is the correct async behavior for inbox-dispatched agents.
- **Stale assertion handling is correct and cites the right pattern.** The old mismatch tests that verified `Surfaced(register_mismatch)` for philosophical UoWs were stale assertions per learnings.md (PR #827). The PR correctly replaces them: normal path tests assert `Prescribed` (no mismatch); gate-mechanism tests use `mock.patch.object` on `_select_executor_type` to force a mismatch artificially. The four updated mismatch tests preserve gate observability (artifact-write blocking, audit-log, steward-log) under the monkeypatched forcing function. The new `test_select_executor_type_by_register.py` (15 tests) covers the mapping as a pure function including the "philosophical with fix keyword → lobster-meta" case that was the original bug.
- **One structural trade-off worth naming:** the mismatch gate tests shifted from integration tests catching realistic production routing bugs to unit tests verifying the gate mechanism works given an artificial override. The new register-mapping tests (`test_select_executor_type_by_register.py`) restore coverage of realistic routing scenarios. Together the coverage is adequate, but the mismatch gate tests no longer serve as the first line of detection for future routing regressions — that role now belongs to the register-mapping tests. This is a deliberate trade-off, not a defect.

**Patterns introduced:** The `_make_inbox_dispatcher(agent_type)` factory producing named module-level closure functions is a clean application of the seam-first abstraction pattern: `agent_type` is baked at factory time, the closure conforms to `SubagentDispatcher`, and module-level naming enables monkeypatching in tests. This pattern generalizes: any future executor type that needs the inbox path adds one line (`_dispatch_via_inbox_<type> = _make_inbox_dispatcher("<type>")`) without modifying the dispatch function itself. The declarative `_REGISTER_TO_EXECUTOR_TYPE` dict lookup replaces a heuristic with a field check, which is the principle-3 golden structure for routing decisions in this codebase.

**What this forecloses:** Nothing structural. `lobster-ops` remains in the dispatch table but is not reachable through `_select_executor_type` (no register maps to it). This pre-existed this PR. The keyword-matching approach is fully replaced — any future routing behavior must go through `_REGISTER_TO_EXECUTOR_TYPE` or `_EXECUTOR_TYPE_TO_DISPATCHER`, which is the correct structural anchor.

**Opportunity cost note:** This directly serves `current_focus.this_week.primary` ("fix executor dispatch starvation") — philosophical and human-judgment UoWs previously surfaced as `register_mismatch` and would not execute; they now dispatch to the correct agent. No competing work is deferred.
...
### [2026-04-22] PR #851 — feat: WOS activation — heartbeat integration, health check, metabolic digest

**Vision alignment:** The adversarial prior — this implementation is solving the wrong problem, or solving the right problem in a direction that forecloses better paths — has partial traction here. The three gaps addressed (heartbeat activation, starvation detection, metabolic observability) are all named in the WOS audit (wos-audit-20260422.md) as real gaps. They are not imaginary problems. The question is sequencing and approach. Vision.yaml current_focus.primary is "WOS execution health: fix executor dispatch starvation, resolve RALPH Cycle 7 failures, drain 106 proposed UoWs." Gap 2 from the audit — starvation root cause — is explicitly named as "undiagnosed." This PR ships detection infrastructure for downstream symptoms (stale heartbeats, stalled UoWs, outcome category distribution) but does not diagnose why the pipeline is starving. A more direct path would have traced dispatch-boundary.jsonl and RALPH Cycle 7 failure modes before adding observability layers. The heartbeat instruction approach (Gap 1) encodes a behavioral requirement as prompt text, which conflicts with vision.yaml principle-3 ("determinism over judgment for conditionals — if-then logic and field checks are code, not LLM instructions"). A structural alternative — a scaffolding wrapper that writes heartbeats on behalf of the agent — would not rely on agent compliance with a prompt section. The metabolic digest and health check are correct additions but will produce maximally noisy signal during the initial deployment period: every active UoW will appear as stall_type=no_heartbeat (since agents don't call write_heartbeat yet), and the metabolic digest will surface mostly shit outcomes until the pipeline produces completions. These are known limitations the PR acknowledges; they do not make the work wrong, but they do mean the signal value is deferred.

**Alignment verdict:** Questioned

**Quality finding:**
- **Health check SQL is correct and structurally sound.** The `query_starvation_candidates` cutoff uses `created_at <= ?` which correctly includes UoWs older than the threshold. The stale-heartbeat query is correctly split: NULL heartbeat_at → stall_type=no_heartbeat (correctly conservative for legacy UoWs from before PR #848); non-NULL heartbeat_at with staleness > heartbeat_ttl + buffer → stall_type=heartbeat. The boundary condition (fresh heartbeat within TTL is not reported) is correctly implemented and tested. The long-running query correctly handles started_at IS NULL fallback to created_at, which covers legacy UoWs. The executor liveness check via log file mtime is a reasonable proxy — not perfect (mtime updates even on log writes that aren't heartbeat runs), but acceptable for the 10-minute stale threshold given the 3-minute executor cycle.
- **classify_uow priority semantics: seed before pearl is correct for this use case but introduces a false-positive surface.** The test `test_seed_takes_priority_over_pearl` verifies "issue referenced in pr" → seed. This is correct: when a UoW opens a PR that references an issue, the seed signal (issue) is the more conservative classification — it doesn't assert code was produced. However, "issue" is a substring of many common English phrases ("issue tracker", "this issue", "no issue") that could appear in close_reason without signaling that a new GitHub issue was created. The seed_keywords set lacks precision: the classification function treats any close_reason containing "issue" as seed-producing work, which will inflate seed counts for UoWs that merely mention the source issue in their close_reason without creating a new one. A more precise signal would be "opened issue #N" or "filed issue" rather than the bare substring "issue".
- **Test constants are re-declared from the spec rather than imported from the implementation.** The comment "Constants — must match the spec, not derived from implementation" (line 1183) explains the intent: the author wanted spec-independence. But this is the pattern named in learnings.md (PR #848): locally-declared mirror of a production constant creates a divergence risk. If `STARVATION_THRESHOLD_HOURS`, `ALERT_THRESHOLD_HOURS`, or `HEARTBEAT_STALE_BUFFER_SECONDS` change in the production module, the test constants do not update, causing tests to silently validate against stale values. The intended spec-independence is correctly motivated but the mechanism (re-declaration) is wrong — the constants should be imported from the production module and the spec constraint should be expressed as a comment on the production constant. This is a named learnings.md failure mode applied deliberately in this PR; the intent is understandable but the execution is technically the same risk.
- **Migration 83 is idempotent and correctly structured.** The `grep -qF "$WOS_HEALTH_MARKER"` check before adding each cron entry is the correct idempotency pattern consistent with previous migrations. The cron expressions are correct (every 6h: `0 */6 * * *`; daily at 09:00 UTC: `0 9 * * *`). The `|| warn` fallback on cron-manage.sh failure is correct: it logs the failure without blocking the migration from incrementing or the overall upgrade from succeeding. The hardcoded `LOBSTER_DIR` and `LOBSTER_WORKSPACE` variable references are consistent with the existing migration convention.

**Patterns introduced:** The `query_*/classify_*/format_*/write_*` functional decomposition (pure readers + pure classifiers + pure formatters + side-effecting writers at the boundary) is a clean and reusable structure for future cron-direct diagnostic scripts. Side effects are isolated to `_write_inbox_alert`, `_write_inbox_digest`, and `write_health_report` — all other functions are pure and testable without mocking. This functional-boundary decomposition is worth propagating to any future Type C scheduled-task script.

**What this forecloses:** The heartbeat instruction approach (prepending a prompt section) may establish a precedent for encoding behavioral requirements as prompt text rather than as code. If this approach is adopted broadly, it reduces the structural prevention guarantee that principle-3 requires. No single design path is foreclosed, but the pattern has a lower ceiling than structural enforcement.

**Opportunity cost note:** The starvation root cause (Gap 2 from the audit, named as primary constraint in vision.yaml current_focus) remains undiagnosed. This PR ships observability that will surface starvation symptoms more quickly, but does not trace why RALPH Cycle 7 failed or what is causing dispatch starvation. A PR that traced dispatch-boundary.jsonl, RALPH failure mode, and proposed a diagnostic would more directly serve current_focus.primary.

**VERDICT: APPROVED**

---

### [2026-04-22] PR #853 — feat: WOS heartbeat structural enforcement — cron sidecar

**Vision alignment:** The adversarial prior — this implementation is solving the wrong problem, or solving the right problem in a direction that forecloses better paths — finds no foothold here. PR #851 was questioned for encoding heartbeat requirements as prompt text (principle-3 violation: behavioral requirements as prompt text rather than code). This PR is the structural enforcement path that #851 could not take: a cron-driven sidecar that writes heartbeats for all in-flight UoWs regardless of agent compliance. The architectural premise (executor.py exits immediately after writing the async inbox dispatch, so an in-process thread would die before writing heartbeats) is correct and follows directly from how executor-heartbeat.py works as a Type C cron-direct script. The cron sidecar (Option B) is the minimum viable structural fix, not over-engineering. Running before the `execution_enabled` gate is deliberately correct: in-flight UoWs need heartbeats even when dispatch is paused, otherwise pausing dispatch would cause false stall detection for already-executing UoWs. This directly serves `current_focus.this_week.primary` ("WOS execution health") and applies vision.yaml `principle-1` (structural prevention over reactive recovery) and `principle-3` (determinism over judgment — heartbeat writes are deterministic field updates, not LLM instructions).

**Alignment verdict:** Confirmed

**Quality finding:**
- **`write_heartbeats_for_active_uows()` implementation is correct.** Error isolation is per-UoW (loop continues on individual exceptions). The `HeartbeatSidecarResult` frozen dataclass with `checked/written/skipped/errors` fields extends the golden pattern "classification result as typed frozen dataclass with observability fields" (golden-patterns.md [2026-04-04]) — the observability fields are first-class outputs, not afterthoughts. `_safe_list` isolating the `active` and `executing` queries independently is correct: a failure to list one status does not prevent the other from being checked, which matters for `executing` (a newer status with potentially different DB coverage).
- **Optimistic lock handling is correctly implemented.** `rowcount == 0` → `skipped` correctly reflects that `write_heartbeat()` uses a WHERE clause on `status IN ('active', 'executing')`. Post-transition writes are no-ops at the DB level, not errors. The race test is honest: after direct SQL update to `ready-for-steward`, `registry.list('active')` returns empty, so `checked=0` — the race is tested at the list boundary, which is the correct race surface (the window is between `list()` and `write_heartbeat()`, not between two `write_heartbeat()` calls). The test comment accurately describes this.
- **Test coverage is adequate and uses real Registry instances.** The false-stall prevention test (`test_sidecar_prevents_false_stall_within_ttl`) exercises the actual integration contract: after the sidecar runs, `registry.get_stale_heartbeat_uows()` does not return the refreshed UoW. This is the highest-value test and covers the end-to-end invariant. Error-isolation and count-accuracy tests use MagicMock correctly.
- **One learnings.md risk: `DEFAULT_HEARTBEAT_TTL_SECONDS = 300` and `HEARTBEAT_STALL_BUFFER_SECONDS = 30` are locally redeclared in the test file**, applying the pattern flagged in learnings.md (PR #848 and PR #851): locally-declared mirror of a production constant creates silent divergence risk when the production value changes. The author imports `SIDECAR_WRITES_PER_CYCLE` from the production module (demonstrating awareness of this principle) but does not import the TTL/buffer constants. If these constants are importable from their production module (`registry.py` or `steward_heartbeat.py`), they should be imported rather than redeclared. This is a bounded risk (300s TTL is unlikely to change without an explicit design decision) but is the named failure pattern.

**Patterns introduced:** The `write_heartbeats_for_active_uows()` cron-sidecar function establishes a structural enforcement pattern for any behavioral requirement that cannot be guaranteed at the agent prompt level: push the requirement into a cron-driven script that fires regardless of agent compliance. This is the direct complement to prompt-text behavioral encoding — and the structurally correct alternative when the requirement is a field update, not a semantic judgment. The `registry: object` type annotation (not `Registry`) to avoid circular imports while preserving the duck-typing contract is a clean solution for scheduled-task modules that load before the registry path is configured.
