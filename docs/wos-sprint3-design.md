...
**Success criterion:** When a UoW transitions to `blocked` via `hard_cap`, the following occur atomically: (1) artifacts archived to `orchestration/artifacts/archived/<uow_id>/`, (2) failure trace written to `orchestration/failure-traces/<uow_id>.json` with `reason`, `cycle_count_lifetime`, and `timestamp`, (3) `close_reason = "hard_cap_cleanup"` and `closed_at` set in the registry so the caretaker does not re-open, (4) a comment posted to the source GitHub issue noting the failure trace. A decide-retry after hard_cap_cleanup requires an explicit operator flag — bare `decide-retry` is rejected. PR passes oracle review.
**Dependencies:** PR #687 (`lifetime_cycles` field) must be merged. It was — confirmed in the retro.
...
**Dependencies:** PR #611 (trace.json written at all exit paths) — confirmed merged (Sprint 1). No other dependencies.
