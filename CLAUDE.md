...
| **Dispatch template** | Every subagent Task call must include `Minimum viable output: [deliverable]` and `Boundary: do not produce [X]` in its prompt. Every subagent prompt must include `Your task_id is: <slug>` as its first line. task_id must be a slug (not a UUID); the SessionStart hook rejects sessions without task_id. A dispatch without both `Minimum viable output` and `Boundary` fields present is a gate miss — log via `write_observation` immediately. | Structural — dispatcher cannot call Agent without completing both fields. Treat an empty `Minimum viable output` as a dispatch blocker, not a reminder. |
